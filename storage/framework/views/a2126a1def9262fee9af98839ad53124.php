...
<div class="d-flex justify-content-between align-items-end mb-4">
    <div>
        ...
        <h1 class="h3 fw-bold mb-1"><?php echo e($isEdit ? 'Edit Data SPP' : 'Tambah Tagihan SPP'); ?></h1>
        <p class="text-secondary mb-0">Pilih siswa berdasarkan kelas, nama, dan NIK.</p>
    </div>
    <a href="<?php echo e(route('spp.index')); ?>" class="btn btn-outline-secondary rounded-pill px-4">
    ...
    <div class="col-lg-8">
        <div class="card form-card">
            <div class="card-body p-4">
                <form method="POST" action="<?php echo e($isEdit ? route('spp.update', $spp) : route('spp.store')); ?>">
                    <?php echo csrf_field(); ?>
                        ...
                        <select name="siswa_id" class="form-select form-select-lg" required>
                            ...
                            <?php $__currentLoopData = $siswas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $siswa): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($siswa->id); ?>" <?php if(old('siswa_id', $spp->siswa_id ?? '') == $siswa->id): echo 'selected'; endif; ?>>
                                    <?php echo e($siswa->kelas?->nama ?? '-'); ?> &middot; <?php echo e($siswa->name); ?> &middot; NIK <?php echo e($siswa->nik ?? '-'); ?>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            ...
                            <select name="bulan" class="form-select" required>
                                ...
                                <?php for($m = 1; $m <= 12; $m++): ?>
                                    <option value="<?php echo e($m); ?>" <?php if(old('bulan', $spp->bulan ?? '') == $m): echo 'selected'; endif; ?>><?php echo e($namaBulan[$m]); ?></option>
                                <?php endfor; ?>
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Tahun</label>
                            <input name="tahun" type="number" min="2020" max="2050" value="<?php echo e(old('tahun', $spp->tahun ?? date('Y'))); ?>" class="form-control" required>
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            ...
                            <input name="nominal" type="number" min="0" id="nominalInput" value="<?php echo e(old('nominal', $spp->nominal ?? '')); ?>" class="form-control" required oninput="updatePreview()">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Sudah Dibayar (Rp)</label>
                            <input name="dibayar" type="number" min="0" id="dibayarInput" value="<?php echo e(old('dibayar', $spp->dibayar ?? 0)); ?>" class="form-control" oninput="updatePreview()">
                        </div>
                    </div>

                    <div class="mb-4">
                        <label class="form-label fw-semibold">Jatuh Tempo</label>
                        <input name="jatuh_tempo" type="date" value="<?php echo e(old('jatuh_tempo', $isEdit && $spp->jatuh_tempo ? $spp->jatuh_tempo->format('Y-m-d') : '')); ?>" class="form-control">
                    </div>
                    ...
                    <button class="btn btn-primary btn-lg rounded-pill px-5">
                        <i class="bi bi-check2-circle me-1"></i> <?php echo e($isEdit ? 'Simpan Perubahan' : 'Buat Tagihan'); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\app-portal-sekolah\resources\views\admin\spp-form.blade.php ENDPATH**/ ?>