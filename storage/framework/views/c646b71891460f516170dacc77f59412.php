...
<div class="p-topbar">
    <a href="<?php echo e(route('dashboard')); ?>" class="back"><i class="bi bi-arrow-left"></i> Beranda</a>
</div>
...
<header class="p-hero">
    ...
    <p>
        <?php if(isset($canManage) && $canManage): ?>
            <?php if(isset($isWaliKelas) && $isWaliKelas): ?>
                Untuk wali kelas <b><?php echo e($isWaliKelas->nama); ?></b> dan pembina eskul Anda.
            <?php else: ?>
    ...
    <span class="count-pill"><i class="bi bi-bell"></i> <?php echo e($pengumumans->count()); ?> pengumuman</span>
</header>
...
<nav class="p-filter" id="pFilter">
    <button class="fbtn active" data-f="all"><i class="bi bi-grid-1x2"></i> Semua <small><?php echo e(count($list)); ?></small></button>
...
<main class="p-list" id="pList">
    <?php $__empty_1 = true; $__currentLoopData = $list; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        ...
        ?>
        <article class="p-card <?php echo e($isPrivate ? 'priv' : ''); ?>" data-cat="<?php echo e($cat); ?>">
            <div class="th <?php echo e($isPrivate ? 'ink' : ($item->gambar ? '' : 'paper')); ?>">
                <?php if($item->gambar): ?>
                ...
                <?php else: ?>
                    <i class="bi <?php echo e($isPrivate ? 'bi-person-lock' : ($item->eskul_id ? 'bi-stars' : ($item->kelas_id ? 'bi-people-fill' : 'bi-megaphone'))); ?> big-ico"></i>
                <?php endif; ?>
                <span class="stage-badge">
                    <i class="bi <?php echo e($isPrivate ? 'bi-lock-fill' : ($item->kelas_id ? 'bi-people-fill' : ($item->eskul_id ? 'bi-stars' : 'bi-globe2'))); ?>"></i>
            ...
            <div class="bd">
                <div class="p-meta">
                    <span class="p-date"><i class="bi bi-calendar3"></i> <?php echo e($item->created_at->format('d M Y')); ?></span>
                    <div style="display:flex;align-items:center;gap:6px;">
                        <?php if($unread): ?><span class="unread"></span><?php endif; ?>
                        <span class="p-tag <?php echo e($cat === 'umum' ? 'tag-umum' : ($cat === 'kelas' ? 'tag-kelas' : ($cat === 'eskul' ? 'tag-eskul' : 'tag-privat'))); ?>">
                            <i class="bi <?php echo e($cat === 'umum' ? 'bi-globe2' : ($cat === 'kelas' ? 'bi-people-fill' : ($cat === 'eskul' ? 'bi-stars' : 'bi-lock-fill'))); ?>"></i>
                            <?php echo e($cat === 'pribadi' ? 'Pribadi' : ($cat === 'kelas' ? 'Kelas' : ($cat === 'eskul' ? 'Eskul' : 'Umum'))); ?>
                ...
                <p class="excerpt"><?php echo e($item->isi); ?></p>

                <div class="p-foot">
                    <div class="p-author">
                        <span class="avatar"><?php echo e($initial); ?></span>
                        <div class="who">
                            <b><?php echo e($item->user->name ?? 'Administrator'); ?></b>
                            <span>
                                <?php if($item->tanggal_acara): ?>
                                    <i class="bi bi-calendar-event"></i> <?php echo e($item->tanggal_acara->format('d M Y')); ?>
                    ...
                    <?php if(isset($canManage) && $canManage && $item->user_id == ($user->id ?? null)): ?>
                        <div class="p-actions">
                            <a href="<?php echo e(route('pengumuman.edit', $item)); ?>" class="act" title="Edit"><i class="bi bi-pencil"></i></a>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
...
<?php if(isset($canCreate) && $canCreate): ?>
    <a href="<?php echo e(route('pengumuman.create')); ?>" class="p-fab" title="Buat pengumuman" style="bottom:90px;"><i class="bi bi-plus-lg"></i></a>
<?php endif; ?>
