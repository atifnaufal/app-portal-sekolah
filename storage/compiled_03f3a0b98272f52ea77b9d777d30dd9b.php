...
<div class="page-header">
    <a href="<?php echo e(route('tugas.index')); ?>" class="btn btn-light rounded-circle p-0 d-flex align-items-center justify-content-center" style="width: 40px; height: 40px;">
...
<div class="page-container px-3">
    <header class="mobile-hero" style="border-radius: 30px; margin-bottom: 25px; background: linear-gradient(135deg, #246bfe, #1e293b);">
        <div class="eyebrow" style="color: rgba(255,255,255,0.7);"><?php echo e($tugas->tipe === 'form' ? 'FORMULIR ONLINE' : 'PENGIRIMAN FILE'); ?> · <?php echo e($tugas->kelas->nama); ?></div>
        <div class="hero-title mt-2 text-white" style="font-size: 24px;"><?php echo e($tugas->judul); ?></div>
        <div class="mt-3">
            <span class="badge bg-white bg-opacity-20 rounded-pill px-3 py-2 fw-normal" style="font-size: 11px;">
                Batas: <?php echo e($tugas->batas_pengumpulan?->format('d M Y') ?? 'Terbuka'); ?>
    ...
    <div class="card ai-card mb-4">
        <div class="card-body p-4">
            <div class="d-flex align-items-center gap-2 mb-3">
                <div class="avatar" style="width:30px; height:30px; font-size: 11px;"><?php echo e(strtoupper(substr($tugas->user->name,0,1))); ?></div>
                <div class="small fw-bold"><?php echo e($tugas->user->name); ?> <span class="text-muted fw-normal">· Pengajar</span></div>
            </div>
            <p class="text-secondary small" style="line-height: 1.6; white-space: pre-line;"><?php echo e($tugas->deskripsi ?: 'Baca instruksi dengan teliti sebelum mengerjakan.'); ?></p>
            ...
            <?php if($tugas->lampiran): ?>
                <div class="mt-4 p-3 border rounded-4 d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <div class="small fw-bold text-truncate" style="max-width: 150px;"><?php echo e($tugas->lampiran_nama); ?></div>
                            <div class="x-small text-muted uppercase">DOKUMEN PDF</div>
                        </div>
                    </div>
                    <a href="<?php echo e(asset('storage/'.$tugas->lampiran)); ?>" target="_blank" class="btn btn-primary btn-sm rounded-pill px-3">Buka</a>
                </div>
            <?php endif; ?>
        </div>
    </div>
        ...
        <?php if($tugas->tipe === 'form'): ?>
            ...
            <div class="card ai-card">
                <div class="card-body p-4">
                    ...
                    <form method="POST" action="<?php echo e(route('tugas.submit', $tugas)); ?>" id="formTask">
                        <?php echo csrf_field(); ?>
                        <?php $__currentLoopData = $formData; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $q): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    @if($item->jawaban_form)
                        ...
                        <div class="p-3 rounded-4 mb-3" style="background: #f8fafc;">
                            ...
                            <?php $__currentLoopData = $formData; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $qi => $q): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                ...
                                <div class="mb-2">
                                    <div class="x-small fw-bold text-dark"><?php echo e($qi + 1); ?>. <?php echo e($q['text'] ?? ''); ?></div>
                                    <div class="x-small text-secondary" style="white-space: pre-line;"><?php echo e(is_array($ans) ? (implode(' • ', array_filter($ans)) ?: '— tidak dijawab —') : ($ans && $ans !== '' ? $ans : '— tidak dijawab —')); ?></div>
                                </div>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>
                    <?php endif; ?>
                    <?php if($item->catatan && !$item->jawaban_form): ?>
                        <div class="x-small text-secondary mb-3 fst-italic">"<?php echo e($item->catatan); ?>"</div>
                    <?php endif; ?>

                    <form method="POST" action="<?php echo e(route('tugas.review', $item)); ?>" class="mt-2 pt-2 border-top">
                        <?php echo csrf_field(); ?>
                        <div class="row g-2">
                            <div class="col-4"><input name="nilai" type="number" step="0.01" class="form-control form-control-sm" placeholder="Nilai" value="<?php echo e($item->nilai); ?>" required></div>
                            <div class="col-8"><input name="feedback_guru" class="form-control form-control-sm" placeholder="Catatan/Feedback" value="<?php echo e($item->feedback_guru); ?>"></div>
                        </div>
                        <div class="form-check form-switch mt-2">
                            <input class="form-check-input" type="checkbox" role="switch" name="revisi_aktif" id="revisi_<?php echo e($item->id); ?>" value="1" <?php if($item->revisi_aktif): echo 'checked'; endif; ?>>
                            <label class="form-check-label x-small" for="revisi_<?php echo e($item->id); ?>">Minta siswa revisi jawaban</label>
                        </div>
