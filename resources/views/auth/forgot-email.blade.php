    ...
    <div class="p-4 p-md-5">
        ...
        @if(session('found_email'))
            <div class="result-box text-center animate__animated animate__fadeIn">
                ...
                <div class="h5 fw-bold text-dark">{{ session('found_email') }}</div>
                <div class="mt-3">
                    <a href="{{ route('login', ['email' => session('found_email')]) }}" class="btn btn-success btn-sm rounded-pill px-3">Gunakan untuk Login</a>
                </div>
            </div>
        @endif

        @if(session('error'))
            <div class="alert alert-danger border-0 rounded-4 small mb-4">{{ session('error') }}</div>
        @endif
