...
class JurusanController extends Controller
{
    ...
    {
        return view('jurusan.index', ['jurusans' => Jurusan::withCount('mahasiswa')->latest()->get()]);
    }

    public function create(): View
    {
        return view('jurusan.form', ['jurusan' => new Jurusan]);
    }

    public function store(Request $request): RedirectResponse
    {
        Jurusan::create($request->validate(['kode' => ['required', 'max:20', 'unique:jurusan,kode'], 'nama' => ['required', 'max:255']]));
    ...
    {
        $jurusan->update($request->validate(['kode' => ['required', 'max:20', 'unique:jurusan,kode,'.$jurusan->id], 'nama' => ['required', 'max:255']]));
