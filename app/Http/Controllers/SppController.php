...
class SppController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        NotificationHelper::send($siswa->id, 'Tagihan SPP Baru', 'Tagihan SPP bulan ' . $data['bulan'] . '/' . $data['tahun'] . ' telah diterbitkan.', route('spp.index'), 'billing');
    ...
    public function remind(Request $request, Spp $spp): RedirectResponse
    {
        ...
        $pesan = 'SPP bulan ' . $spp->bulan . '/' . $spp->tahun . ' masih memiliki kekurangan Rp ' . number_format($spp->kekurangan, 0, ',', '.');
